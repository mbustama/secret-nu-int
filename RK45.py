...
def Runge_Kutta(E, h, g, M, m, err):
    ...
    for i in np.arange(E.size):
        ...
        while z >= 0:
            ...
            n_new[i] = Decimal(n_previous[i]) + a
            
            n_5order[i] = Decimal(n_previous[i]) + b
            
            print(repr(n_5order[i]), repr(n_new[i]))
            
            #s = (err*(-h)/(2*abs(n_5order[i]-n_new[i])))**0.25 
            
            print("Individual first", 25*k1/216, 1408*k3/2565, 2197*k4/4104, k5/5)
            ...
            if count == 0:
                ...
                #s = (err*h/(2*(abs(n_5order[i] - n_new[i]))))**0.25 
                s = (err*h/(2*abs(float(n_5order[i]-n_new[i]))))**0.25 
